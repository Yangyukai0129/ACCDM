...
from module.diffusion_baseline import ddim_inference, make_beta_schedule
...
BATCH_SIZE     = 32
NUM_STEPS      = 100
MODEL_PATH     = './checkpoints/CDM_window5.pth'
SAVE_DIR       = './eval_results_CDM'
...
def compute_ssim_p95(pred, true, threshold):
    """
    只在真實值超過格點 P95 閾值的樣本（超過比例 > 0）計算 SSIM。
    """
    scores = []
    for i in range(len(true)):
        mask = true[i] > threshold
        ...
        scores.append(ssim(true[i], pred[i], data_range=dr))
...
def print_metrics(results):
    header = (f"\n{'':>10} {'RMSE mean±std':>20} {'RMSE P95':>18} "
              ...
              f"{'SSIM mean±std':>20} {'CSI mean±std':>18}")
    print(header)
    print('-' * 140)
...
if __name__ == '__main__':
    ...
    beta = ckpt['beta'] if 'beta' in ckpt else make_beta_schedule()[0]
    ...
    with torch.no_grad():
        for i, (cond, target, year) in enumerate(test_loader):
            cond_np = cond.numpy()
            cond    = cond.to(device)
            pred    = ddim_inference(model, cond, beta, device,
                                     eta=1.0, num_steps=NUM_STEPS)
    ...
    train_true_list = []
    with torch.no_grad():
        for _, target, _ in train_loader_p95:
    ...
    results['Overall'] = {
        ...
        'nRMSE': {'mean': overall_nrmse.mean(), 'std': overall_nrmse.std()},
        ...
        'CSI'  : {'mean': np.nanmean(csi_m_list), 'std': np.nanmean(csi_s_list)},
    }
    ...
    for label, step in TARGET_STEPS.items():
        ...
        nrmse_s = compute_nrmse(all_pred_t[:, step], all_true_t[:, step])  # ← 加
        csi_m, csi_s = compute_csi(all_pred_t[:, step], all_true_t[:, step], threshold_all[step])  # ← 加
        results[label] = {
            'RMSE': {'mean': rmse_s.mean(), 'std': rmse_s.std(), 'p95_mean': p95_rm, 'p95_std': p95_rs},
            'nRMSE': {'mean': nrmse_s.mean(), 'std': nrmse_s.std()},
            'nRMSE': {'mean': nrmse_s.mean(), 'std': nrmse_s.std()},
            'MAE' : {'mean': mae_s.mean(),  'std': mae_s.std(),  'p95_mean': p95_mm, 'p95_std': p95_ms},
            'SSIM': {'mean': ssim_s.mean(), 'std': ssim_s.std(), 'p95_mean': p95_sm, 'p95_std': p95_ss},
            'CSI'  : {'mean': csi_m, 'std': csi_s},
        }
    ...
    for label, step in TARGET_STEPS.items():
        ...
        csi_p, csi_ps = compute_csi(last_input, all_true_t[:, step], threshold_all[step])  # ← 加
        print(f"...   {csi_p:.4f} ± {csi_ps:.4f}")  # ← 加
        ...
        for row, (label, step) in enumerate(plot_steps):
            ...
            for ax, data, cmap, vlo, vhi, title, clabel in [
                ...
                im = ax.imshow(data, origin='upper', cmap=cmap,
                               vmin=vlo, vmax=vhi, extent=extent, aspect='auto')
                ax.set_title(title)
                ax.set_xlabel('Longitude')
                ax.set_ylabel('Latitude')
                ax.set_xticks(lon_ticks)
                ax.set_yticks(lat_ticks)
                ax.set_xticklabels([f"{abs(v)}°W" if v < 0 else f"{v}°E" for v in lon_ticks])
                ax.set_yticklabels([f"{v}°N" for v in lat_ticks])
