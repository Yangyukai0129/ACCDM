...
from module.csdi_diffusion import csdi_inference
...
BATCH_SIZE     = 32
NUM_STEPS      = 100
MODEL_PATH     = './checkpoints/csdi.pth'
SAVE_DIR       = 'eval/eval_results_csdi'
...
if __name__ == '__main__':
    ...
    config = ckpt['config']
    model = CSDITransformer(config).to(device)
    ...
    with torch.no_grad():
        for i, (cond, target, year) in enumerate(test_loader):
            ...
            cond    = cond.to(device)
            beta = ckpt['beta']
            pred = csdi_inference(model, cond, beta, device, eta=0.0, num_steps=NUM_STEPS)
